...
> `/haqqq`는 필요한 경우에만 최대 12개(호출당 최대 4개)까지 심층 질문을 제시하고, 명시적 동의가 있을 때만 최대 20개까지 확장합니다. 실행은 기본적으로 직렬이며, 병렬 전문 탐색은 `/cast`의 역할입니다.
...
### 2단계: 공통 엔진이 적절한 모델을 순차 배정합니다

한 명의 AI에게만 물어보면,
그 AI의 편향이 그대로 결과에 반영됩니다.

`/haqqq`는 질문 깊이와 모델 라우팅을 분리합니다. 종합 판단은 Sol/Opus, 표준 구현은 Terra/Sonnet, 단순 탐색·국소 변경은 Luna/Haiku가 담당할 수 있으며, 런타임 기능에 따라 폴백합니다. 기본값은 `max_concurrency=1`입니다.
필요한 역할별 결과를 순차적으로 모아서 종합하며, 검증 단계에서 관점 차이를 점검합니다. 여러 전문 탐색을 실제로 병렬 실행해야 하는 경우에는 `/cast`를 사용합니다.
