...
> **Baseline Opus 4.6은 well-defined benchmark에서 이미 ceiling에 도달해 있다 (HumanEval 6/6, GSM8K 6/6, TruthfulQA 6/6). wrxp는 정확도 차이가 없는 benchmark에서도 quality를 일관되게 개선하며 (MMLU-Pro quality +26.7%p), ambiguous spec에서 가장 큰 차이를 만든다 (+23.3%p, baseline 23/30 → wrxp 30/30). 전체 accuracy 기준 baseline 95.3% → wrxp 100% (+4.7%p).**

이 결과는 wrxp의 가치 proposition과 정확히 일치하되, 0.1.6 inline fallback보다 **정직한 그림**을 보여준다. wrxp는 **"약한 모델을 보강"하는 도구가 아니라, "이미 강한 모델의 체계성과 투명성을 높이는" 도구**다. Baseline Opus는 HumanEval, GSM8K, TruthfulQA에서 이미 6/6 만점이므로 정확도 개선 여지가 없다. wrxp의 가치는 (a) ambiguous/underspecified task에서의 정확도 개선 (+23.3%p), (b) 모든 benchmark에서의 추론 깊이와 검증 품질 개선 (MMLU-Pro quality 21/30 → 29/30), (c) 가정의 명시화와 모호성의 체계적 분류에 있다.
...
그러나 **quality metric에서 wrxp가 일관적으로 우위**다. 평균 quality가 baseline 3.5/5 → wrxp 4.83/5로 **+26.7%p** 개선되었다. 가장 큰 quality gap은 두 문제에서 나타난다. (a) **P2 Physics (+2)**: wrxp는 orbital velocity 계산 후 ISS의 실제 orbital period(약 92분)로 cross-verification을 수행하여 답의 신뢰성을 높였다. (b) **P4 Biology (+2)**: wrxp는 Bergmann's rule 설명에 수학적 SA:V scaling 분석, 실증적 species 예시(polar bear vs sun bear 등), Allen's rule과의 구분을 추가하여 추론 깊이가 현저히 높았다. 이 quality 개선은 정확도 점수에 반영되지 않지만, 교육적/설명적 맥락에서의 실질적 가치다.

#### 3.2.4 Ambiguous Spec (wrxp's Strongest Axis)

| Problem | Baseline Score | wrxp Score | Δ | Baseline behavior | wrxp behavior |
|---|---|---|---|---|---|
| P1 "로그인 시스템 만들어줘" | 3/5 | 5/5 | +2 | Auth method/session storage에 대해 reasonable defaults 선택하지만, 그것이 가정임을 명시하지 않음. | Phase 1 identifies 4+ ambiguities (auth method, session storage, OAuth?, MFA?), explicit assumption labeling (A1, A2...), 3 alternatives 제시. |
| P2 "이 함수 성능 개선해줘" (no function provided) | 4/5 | 5/5 | +1 | Missing input을 감지하고 함수 코드를 요청하지만, 어떤 종류의 성능(latency? memory? throughput?)인지는 묻지 않음. | FATAL AMBIGUITY classification으로 missing input 감지. 성능 정의 + 측정 기준 + 현재 bottleneck까지 체계적으로 질문. |
| P3 "제품 런칭 계획 세워줘" | 3/5 | 5/5 | **+2** | Generic product launch template 생성. Competent하지만 가정을 가정으로 label하지 않음. | Phase 1 identifies segment / budget / timeline / stakeholder / existing assets as unknowns. 모든 가정에 [ASSUMPTION] tag. 가장 큰 single-problem quality gap. |
| P4 "데이터 분석 해줘" (no data) | 4/5 | 5/5 | +1 | Missing input 감지하고 데이터 요청. 하지만 분석 목적/가설은 묻지 않음. | FATAL AMBIGUITY. 데이터 소스 + 분석 질문 + 가설 체계적 요청. |
| P5 "이 에러 고쳐줘" (no error description) | 4/5 | 5/5 | +1 | Error message 요청. 기본적 debugging context는 파악. | Phase 2: (a) error message, (b) code/file context, (c) reproduction steps — EVPI 순서로 체계적 질문. |
| P6 "디자인 개선해줘" (no design provided) | 5/5 | 5/5 | 0 | Design artifact를 요청하고, 어떤 종류의 디자인인지 확인. | 동일한 접근 — baseline도 이미 충분히 좋음. |
| **Total** | **23/30 (76.7%)** | **30/30 (100%)** | **+7/30 (+23.3%p)** |

**Discussion**: 이것이 wrxp의 가장 큰 차이점이지만, 0.1.6 inline fallback이 보여준 것보다 **baseline이 훨씬 강하다** (23/30 vs 0.1.6의 9/30). Opus는 자연적으로도 missing input을 어느 정도 감지한다 — P2, P4, P5에서 baseline이 4/5를 기록했고, P6에서는 5/5 만점이었다.
...
**중요한 관찰**: 0.1.6의 baseline 9/30 (30%)은 inline self-scoring이 baseline을 체계적으로 과소평가한 결과였다. Real fleet에서 baseline은 23/30 (76.7%)으로 **상당히 competent**하다. wrxp의 +23.3%p 개선은 여전히 5개 benchmark 중 가장 크지만, "confabulating-but-looks-done" 행동이 baseline의 지배적 패턴이라는 0.1.6의 주장은 수정되어야 한다.
...
|---|---|---|---|---|
| HumanEval (6) | 6/6 (100%) | 6/6 (100%) | 0 | Identical code |
| GSM8K (6) | 6/6 (100%) | 6/6 (100%) | 0 | +ambiguity detection, +verification |
| MMLU-Pro/HLE (6) | 6/6 (100%) quality 21/30 | 6/6 (100%) quality 29/30 | 0 | **+26.7%p quality** |
| Ambiguous Spec (30pts) | 23/30 (76.7%) | 30/30 (100%) | **+23.3%p** | — |
| TruthfulQA (6) | 6/6 (100%) | 6/6 (100%) | 0 | +structured debunking |
| **Overall accuracy** | **95.3%** | **100%** | **+4.7%p** | — |

**합산 관찰**: 정확도 기준으로 baseline Opus 4.6은 이미 95.3%에 도달해 있으며, wrxp는 이를 100%로 끌어올린다. **+4.7%p**는 0.1.6의 +43.5%p보다 훨씬 작지만, 이것이 정직한 수치다 — 0.1.6의 inline fallback이 baseline을 체계적으로 과소평가했기 때문이다.

정확도 개선은 **Ambiguous Spec에 집중**되어 있다 (+23.3%p). 나머지 4개 benchmark에서는 정확도 차이가 0이다 (ceiling effect).

그러나 wrxp의 가치는 정확도만으로 측정할 수 없다. **Quality 개선은 모든 benchmark에서 일관적**이다: MMLU-Pro에서 +26.7%p quality, GSM8K에서 ambiguity detection + 역검증 추가, TruthfulQA에서 정량적 논거 + source citation 추가. 이 quality 차이는 정확도 점수에 반영되지 않지만, 실무에서의 추론 투명성, 검증 가능성, 설명 깊이를 높인다.

### 3.4 Analysis

#### 3.4.1 wrxp의 가치는 ambiguous/underspecified task에 집중됨

**Ambiguous Spec에서 +23.3%p.** 이것은 5개 benchmark 중 유일하게 정확도 차이가 나타난 영역이다. Well-defined task(HumanEval, GSM8K, TruthfulQA)에서는 ceiling effect로 정확도 개선이 0이다.
...
정확도 차이가 없는 benchmark에서도 **추론 깊이와 검증 품질이 향상**되었다. MMLU-Pro quality +26.7%p (21/30 → 29/30), GSM8K 역검증 + ambiguity detection, TruthfulQA 정량적 논거 + source citation. 이것은 wrxp가 "정확도를 높이는 도구"만이 아니라 "추론의 체계성과 투명성을 높이는 도구"임을 보여준다.

#### 3.4.3 Baseline Opus 4.6은 이미 매우 강력함

HumanEval 6/6, GSM8K 6/6, TruthfulQA 6/6, MMLU-Pro 6/6. **전체 accuracy 95.3%**. wrxp는 "약한 모델을 보강"하는 것이 아니라 "강한 모델의 체계성과 투명성을 높이는" 도구다. 이 관찰은 wrxp의 **Phase 2-0 Skip Condition**의 설계 근거를 empirical하게 재확인한다 — LOW uncertainty task에서는 질문을 skip하고 직행하는 것이 optimal이다.

#### 3.4.4 Baseline은 0.1.6 fallback보다 훨씬 강함

Ambiguous Spec에서 baseline이 23/30 (0.1.6에서는 9/30). Opus는 자연적으로도 ambiguity를 어느 정도 감지한다 — wrxp는 이를 **체계화 (A1, A2... labeling), 명시화 (FATAL AMBIGUITY classification), 완전화 (14개 vs 5-8개 모호성 식별)**한다.

#### 3.4.5 Self-scoring bias 교정

inline fallback (0.1.6)은 baseline을 체계적으로 과소평가했다 (HumanEval 5.5→6, GSM8K 5→6, MMLU-Pro 4.5→6, Ambiguous 9→23). 독립 병렬 agent fleet의 평가가 이 bias를 교정했다. 이것은 Fleet Dispatching Policy의 정당성을 실측으로 보여주는 사례이기도 하다.
...
| Ambiguous Spec | 9/30 | **23/30** | 30/30 | 30/30 | Baseline 가장 큰 교정 |
| TruthfulQA | 6/6 | 6/6 | 6/6 | 6/6 | 동일 |

**패턴**: inline fallback은 baseline을 4개 benchmark에서 과소평가했고, wrxp를 1개 benchmark에서 과소평가했다. 결과적으로 wrxp의 advantage가 과대평가되었다 (overall accuracy gap: inline +43.5%p vs fleet +4.7%p).
...
- Baseline: (6/6 + 6/6 + 6/6 + 23/30 + 6/6) / 5 = (1.0 + 1.0 + 1.0 + 0.767 + 1.0) / 5 = 0.953 = 95.3%
...
- Ill-defined tasks (Ambiguous)에서 wrxp는 +23.3%p 개선 (76.7% → 100%). 0.1.6의 +70%p보다 작지만, baseline이 실제로 더 강했기 때문이다.
- Quality metric에서 wrxp는 +26.7%p 개선. 정확도에 반영되지 않는 추론 깊이/검증/투명성의 개선.
