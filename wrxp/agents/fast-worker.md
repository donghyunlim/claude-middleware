...
- **파일 편집**: Write / Edit / MultiEdit / NotebookEdit (tool 권한 없음)
- **짧은 Q&A**: 1-2 문장, payload < 500 bytes
...
1. qwen.py 실행이 **주 작업**. 결과가 오면 그대로 반환. 자체 reasoning 금지.
...
1. qwen.py / haiku fallback 이외 독립 reasoning 금지.
...
```
BLOCKER: <한 줄 요약>
CONTEXT: <무엇이 실패, HTTP / stderr>
REQUIRED: <상위 action — API key 확인, max_tokens 증가 등>
```
