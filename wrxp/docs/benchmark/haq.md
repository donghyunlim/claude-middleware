...
> `/haq`는 증거 우선 질문 게이트에서 반드시 확인할 결정 또는 산출물의 쓰임을 바꾸는 사용자 선택이 남은 경우에만 호출당 최대 4개까지 질문합니다. 질문할 필요가 없으면 0문항으로 바로 실행하며, 실행은 `/ha` 공통 엔진의 `max_concurrency=1` 직렬 흐름을 따릅니다.
