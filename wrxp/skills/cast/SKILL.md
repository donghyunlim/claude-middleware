...
# /cast — Universal Reasoning & Execution Pipeline

This skill produces high-quality outputs for any task type by combining deep reasoning (Opus) with delegated execution (Haiku). It is the canonical 7-phase engine (Phase 0-6) that /castq, /castqq, /castqqq invoke as thin shims.
...
1. **Direct invocation (`/cast ...`)**: defaults to `depth_budget: 0`, meaning Phase 2 (Questioning) is **skipped** unless Phase 1 detects HIGH uncertainty. This honors the principle that 77% of requests already have a clear top-intent (Amazon Alexa research) and that simple queries waste 19-42 seconds on unnecessary reasoning (Stop Overthinking, arXiv:2503.16419).

2. **Tier shim invocation (`/castq`, `/castqq`, `/castqqq`)**: a config block is passed in via `$ARGUMENTS`. Parse the following keys at the top of the requirements:

```
depth_budget: <0-4 | 5-8 | 9-12-up-to-20>
question_rounds: <1 | 2 | 3-5>
max_budget: <4 | 8 | 20>
tier: <castq | castqq | castqqq | direct>
```
...
|---|---|---|---|---|
| **/cast (default)** | 중상급 (mid-upper) | 5 agents | 15 | Standard |
| **/castq** | 중상급 (mid-upper) | 5 agents | 15 | Standard |
| **/castqq** | 상급 (upper) | 8 agents | 24 | Expanded |
| **/castqqq** | 최상급 (top) | 12 agents (up to 20 if critical) | 36-60 | **Unlimited** |

> **Benchmark**: 9개 병렬 리서치 에이전트 dispatch가 /castqqq 표준 동작 수준. 유저가 명시적으로 이 수준을 "최상급"의 기준으로 지정했다.
...
| Complexity | /cast·/castq | /castqq | /castqqq |
|---|---|---|---|
...
**규칙**: Task 복잡도가 낮으면 tier가 높아도 과잉 dispatch 금지. Trivial task에 /castqqq를 호출해도 3-5개 agents로 충분하며, 그 이상은 noise만 증가시킨다. 반대로 /cast이라도 critical task이면 tier 상한(5)까지 전부 사용한다.
...
4. **Tie-breaker (runqqq only)** — agent 결과가 충돌하면 추가 critic agent 1개를 소환하여 adjudicate. 일반 tier는 Opus가 직접 결정.
5. **Budget warning (runqqq)** — 총 병렬 agent 수가 20을 초과하면 사용자에게 경고 후 진행. 무제한 예산이어도 사용자 confirmation 한 번은 거쳐야 한다.
...
- **Oversize on trivial**: README 오타 수정에 /castqqq 12개 agents 파견 — complexity 점수 확인 후 축소.
- **Tier 무시**: /castq가 10개 agents dispatch — tier upper bound 준수, 초과 시 /castqq 이상을 명시적으로 호출하도록 안내.
- **Candidate blindness**: "mapping 표에 없는 agent라서 skip" — 매핑은 힌트일 뿐, runtime에 새로 발견된 유용한 agent는 적극 편입한다.

### Fleet Mode Toggle

- **Default**: /cast이 직접 호출되고 config가 없으면 fleet mode ON, tier=중상급으로 동작한다.
- **Shim-driven**: /castq/castqq/castqqq가 /cast을 호출할 때 `tier` config를 명시하여 해당 tier로 fleet mode ON.
...
- **Forced downgrade**: /castqqq 호출이어도 complexity=trivial이면 fleet 크기가 3-5로 축소된다. Tier는 *상한*이지 *하한*이 아니다.
...
| **MEDIUM** | 1-3 | 3-5 | Enter Phase 2, /castq or /castqq range applies |
| **HIGH** | ≥4 | ≥6 | Enter Phase 2, /castqq or /castqqq range applies |

The caller's `depth_budget` is a CEILING, not a floor. If the caller is /castqqq but the actual uncertainty is LOW, you still skip Phase 2. This is the **Stop Overthinking** principle (arXiv:2503.16419) operationalized.
...
1. `depth_budget == 0` (direct /cast invocation default)
...
**Code**:
- castq: {핵심 기능, 데이터·구조}
- castqq: {핵심 기능, 데이터·구조, 에러 처리, 성능·제약, 통합·의존성}
- castqqq: {핵심 기능, 데이터·구조, 에러 처리, 성능·확장성, 통합·의존성, 보안·검증, 운영·모니터링}

**Writing**:
- castq: {독자·톤, 핵심 메시지}
- castqq: {독자·톤, 분량·형식, 핵심 메시지, 사실·자료, 구성·흐름}
- castqqq: {독자·톤, 분량·형식, 핵심 메시지, 사실·자료, 구성·흐름, 인용·출처, 편집 라운드, 게시 계획}

**Planning**:
- castq: {목표·성공 기준, 핵심 제약}
- castqq: {목표·성공 기준, 제약, 우선순위, 리스크, 이해관계자}
- castqqq: {목표·성공 기준, 제약, 우선순위, 리스크, 이해관계자, 컨틴전시, 의사소통, 측정 지표, 회고 계획}

**Research**:
- castq: {질문 구체화, 자료 출처}
- castqq: {질문 구체화, 기존 지식, 자료 출처, 분석 방법, 결론 형식}
- castqqq: {질문 구체화, 기존 지식, 자료 출처, 분석 방법, 편향 통제, 결론 형식, 재현 가능성, 후속 연구}

**Analysis**:
- castq: {분석 질문, 데이터 출처}
- castqq: {분석 질문, 데이터 출처, 분석 방법, 가정, 해석·결론 형식}
- castqqq: {분석 질문, 데이터 출처, 데이터 품질·전처리, 분석 방법, 가정, 엣지 케이스, 해석, 감도 분석, 표현·시각화, 재현성·동료 검토}

**Decision**:
- castq: {대안들, 평가 기준}
- castqq: {대안들, 평가 기준, 가중치, 시간 압박, 되돌릴 수 있는가}
- castqqq: {대안들, 평가 기준, 가중치, 시간 압박, 되돌릴 수 있는가, 감도 분석, 2차 효과, 의사소통}

**Creative**:
- castq: {주제·정서, 스타일·레퍼런스}
- castqq: {주제·정서, 시점·화자, 스타일·레퍼런스, 길이·포맷, 제약}
- castqqq: {주제·정서, 시점·화자, 스타일·레퍼런스, 길이·포맷, 제약·권리, 오디언스, 반복 계획}

**Learning**:
- castq: {목표 수준, 가용 시간}
- castqq: {현재 수준, 목표 수준, 가용 시간, 선호 학습 방식, 평가 방법}
- castqqq: {현재 수준, 목표 수준, 가용 시간, 선호 학습 방식, 평가 방법, 동기·리마인더, 실패 복구, 진척 추적}
...
- `question_rounds: 1` (castq) — 한 라운드에 모든 질문, 최대 4개
- `question_rounds: 2` (castqq) — 라운드당 4개씩 최대 2라운드
- `question_rounds: 3-5` (castqqq) — 라운드당 4개씩, 기본 3라운드, 필요 시 최대 5라운드
...
4. Print the **User Escalation** message:
   ...
   - 확인이 필요한 항목: [3개 이내]
   - 다음 행동 제안: [restart with /castqq, clarify constraint X, provide data Y]
   ```
...
- **Skill invocation** from /castq, /castqq, /castqqq prepend config; /cast respects config or defaults to depth_budget=0.
- **No re-implementation**: /castq/castqq/castqqq are thin shims; they never redefine Phase 0–6 logic. They only set config + category hints.
