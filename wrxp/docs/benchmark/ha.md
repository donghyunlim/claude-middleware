# /ha — 어떤 작업이든, 체계적으로 처리합니다

> AI에게 일을 시키면, 잘 해주는 날도 있고 영 이상한 결과가 나오는 날도 있죠?
> `/ha`는 어떤 종류의 요청이든 **같은 품질로, 매번 일관되게** 처리하는 범용 실행 명령어입니다.

---

## 이런 분에게 추천합니다

- 코드, 글쓰기, 분석, 리서치 등 **다양한 종류의 작업**을 AI에게 맡기는 분
- AI 결과물의 **품질이 들쭉날쭉**해서 답답했던 분
- 중요한 작업인데 AI가 **거짓 정보(환각)를 만들어낼까 불안**한 분
- 모호한 요청을 넣었을 때 AI가 **알아서 추측하고 틀리는 게 싫은** 분

---

## 어떻게 동작하나요?

### 1단계: 요청을 받으면, 먼저 "이건 어떤 종류의 작업인가?" 자동으로 판단합니다

코드 작성, 디버깅, 리팩토링, 문서화, 테스트, 설정, 데이터 처리, 리서치, 설계
— 총 **9가지 작업 유형**을 자동으로 구분합니다.

110만 건의 실제 사용 데이터를 분석해 분류 기준을 잡았기 때문에,
대부분의 요청을 정확하게 인식합니다.

왜 이게 중요할까요?
코드 작성과 리서치는 접근 방식이 완전히 다릅니다.
작업 유형에 맞는 전문 템플릿을 적용하면
엉뚱한 방향으로 가는 걸 원천 차단할 수 있습니다.

### 2단계: 실행 전에 충분히 생각합니다

작업을 바로 시작하는 대신,
AI가 먼저 **"이 문제를 어떻게 풀어야 하지?"** 하고
구조화된 사고 과정을 거칩니다.

구체적으로는 이런 과정입니다:

- **하위 질문으로 쪼개기**: "이 문제를 풀려면 먼저 뭘 알아야 하지?"
- **여러 접근법 비교**: "방법 A와 방법 B 중 어느 게 나을까?"
- **쉬운 부분부터 점진적으로**: 작은 문제를 먼저 풀고, 그 결과를 활용해 큰 문제 해결

이 사전 추론 과정만으로 추론 품질(깊이, 구조, 검증)이
**+20%p (MMLU-Pro 추론 품질)** 향상되는 것으로 나타났습니다.

### 3단계: 결과를 내놓기 전에 "이거 맞나?" 자기 점검을 합니다

답변을 만든 뒤, AI가 스스로 점검하는 단계를 거칩니다:

- **"방금 내 답변이 서로 모순되지 않나?"**
- **"빠뜨린 건 없나?"**
- **"사실과 다른 정보를 만들어내지 않았나?"**

작업 유형에 따라 점검 방식도 달라집니다:

| 작업 유형 | 점검 방식 |
|---|---|
| 코드 | 테스트 실행으로 검증 |
| 문서 | 정확성 교차 확인 |
| 설정 | 드라이런(dry-run)으로 사전 검증 |
| 분석/리서치 | 출처 및 논리 검증 |

이 자기 점검 기법은 코딩 벤치마크에서 정확도를
**80% -> 91% (HumanEval pass@1)**로 끌어올린 것으로 검증되었습니다.
거짓 정보(환각)도 **50-70% 감소 (장문 생성 환각률, MultiSpanQA F1)** 합니다.

> 📎 Shinn et al. "Reflexion" (2023) arXiv:2303.11366 · Dhuliawala et al. "Chain-of-Verification" (2023) arXiv:2309.11495

### 4단계: 점검에서 문제를 발견하면? 자동으로 다시 시도합니다

점검 결과가 기준에 미달하면,
처음 사고 단계로 돌아가서 **자동으로 재추론**합니다.

실제 결함을 고치는 수정 횟수에는 고정 상한을 두지 않습니다. 대신 남은 수용 기준과 검증 근거를 기준으로 다음처럼 점검합니다:

| 수정 횟수 | 결함이 남았을 때의 처리 |
|---|---|
| 5·15·25… | AI가 원래 목적·계획·수정의 필수성을 점검 |
| 10·20·30… | 현황과 다음 수정안을 요약하고 사용자 판단을 기다림 |
| 그 외 | 새 증거나 측정 가능한 진전이 있을 때 계속 수정 |

모든 기준을 충족하면 배수와 관계없이 완료합니다. 같은 입력과 가설을 새 증거 없이 반복하지 않으며, 선택적 개선과 인접 하드닝은 현재 범위에서 제외합니다. 새 권한·외부 변경·보안 경계 확대는 횟수를 기다리지 않고 즉시 사용자 승인을 받습니다.

### 5단계: 모호성을 증거와 승인 경계로 처리합니다

"이 코드 고쳐줘"처럼 해석이 여러 가지인 요청에서도 직접 `/ha`는 선제 discovery 질문을 하지 않습니다. 먼저 저장소와 도구로 증거를 확인하고, 남으면 안전한 기본값을 기록합니다. 권한·보안·비밀·외부 변경·파괴적·불가역 경계만 즉시 사용자 승인을 받습니다.

이 한 가지 기능만으로
모호한 요청의 정확도가 **33.3% -> 100% (+66.7%p, Ambiguous Spec 정확도)**로 올라갔습니다.

---

## 이전보다 뭐가 좋아지나요?

| 상황 | 기존 AI | `/ha` 사용 시 | 개선 |
|---|---|---|---|
| 모호한 요청 해석 | 근거 없는 추측 | 증거 또는 기록한 기본값으로 진행, 승인 경계는 확인 | **승인 경계 보호** |
| 복잡한 추론 문제 | 얕은 답변, 핵심 누락 | 구조화된 사고 후 실행 | **+20%p (MMLU-Pro 추론 품질)** |
| 코드 작성 후 오류 | 한 번 만들고 끝 | 자기 점검 + 자동 수정 | **+11%p (HumanEval pass@1)** |
| 거짓 정보(환각) | 자신 있게 틀린 답변 | 검증 체인으로 사전 차단 | **50-70% 감소 (장문 생성 환각률)** |
| 작업 유형 혼동 | 모든 요청을 같은 방식으로 처리 | 9가지 유형별 전문 대응 | 질적 개선 |

---

## 주요 수치

| 기능 | 개선 효과 | 검증 지표 |
|---|---|---|
| 모호한 요청 해석 | 정확도 33.3% -> 100% (+66.7%p) | Ambiguous Spec 정확도 |
| 사전 추론 후 실행 | 추론 품질 +20%p (80% -> 100%) | MMLU-Pro 추론 품질 |
| 자기 점검 + 재검토 | 코딩 정확도 80% -> 91% (+11%p) | HumanEval pass@1 |
| 거짓 정보(환각) 감소 | 50-70% 감소 | 장문 생성 환각률, MultiSpanQA F1 |
| 자기 수정 반복 | 평균 +20% 품질 향상 | 7개 태스크 평균 품질 점수 |
| 하위 질문 분해 추론 | 정확도 17.6% -> 60% (+42.4%p) | Bamboogle 정확도 |
| 다단계 분기 탐색 추론 | 정확도 4% -> 74% (+70%p) | Game of 24 성공률 |
| 점진적 해결 | 정확도 16% -> 99.7% (+83.7%p) | SCAN 정확도 |
| 자동 작업 유형 감지 | 9가지 유형 분류 | 1.1M건 사용 데이터 기반 |
| 전체 정확도 | 72.7% -> 100% (+27.3%p) | 5개 벤치마크 종합 |

> 📎 ¹ Shinn et al. "Reflexion" (2023) arXiv:2303.11366
> ² Dhuliawala et al. "Chain-of-Verification" (2023) arXiv:2309.11495
> ³ Madaan et al. "Self-Refine" (2023) arXiv:2303.17651
> ⁴ Press et al. "Measuring and Narrowing the Compositionality Gap" (2022) arXiv:2210.03350
> ⁵ Yao et al. "Tree of Thoughts" (2023) arXiv:2305.10601
> ⁶ Zhou et al. "Least-to-Most Prompting" (2022) arXiv:2205.10625

---

## 사용 예시

```
/ha "이 함수에 에러 핸들링 추가해줘"
```

작업 유형 "코드"로 자동 분류 -> 코드 전문 추론 -> 실행 -> 테스트 검증까지 자동.

```
/ha "우리 서비스의 온보딩 플로우 개선 방안 분석해줘"
```

작업 유형 "리서치/분석"으로 분류 -> 다각도 분석 -> 출처 검증 -> 구조화된 보고서.

요청만 넣으면 작업 유형 판단부터 실행, 검증까지 자동으로 진행됩니다.

---

## 다른 명령어와의 관계

`/ha`는 모든 명령어의 **실행 엔진**입니다.
모든 명령어가 같은 사고 체계를 공유합니다 — 일관된 품질의 비결.
9가지 추론 원칙이 하나의 canonical source에 정의되어 있어,
어떤 명령어를 사용하든 동일한 사고 프레임워크가 적용됩니다.

| 명령어 | 역할 | `/ha`와의 관계 |
|---|---|---|
| `/haq` | 빠른 실행 (0~4개 질문) | 빠른 질문 프리셋으로 `/ha` 실행 |
| `/haqq` | 균형 잡힌 실행 (0~8개 질문) | 표준 질문 프리셋으로 `/ha` 실행 |
| `/haqqq` | 심층 실행 (0~12개, 동의 시 최대 20개 질문) | 심층 질문 프리셋으로 `/ha` 실행 |
| `/breakdown` | 대규모 작업 분해 | 각 하위 작업에 `/ha` 실행 |

각 질문 tier의 질문 수는 상한이며, 필요한 질문이 없으면 0문항으로 진행합니다. 직접 `/ha`는 `none` preset으로 0문항·0라운드이며, 실행 통제 경계만 discovery 예산 밖에서 확인합니다. 최종 실행은 `/ha`가 담당합니다.

---

## 이 명령어를 쓰면 안 되는 경우

- **아주 단순한 작업** (예: 파일 이름 하나 바꾸기)
  — 직접 하는 게 더 빠릅니다.

- **대규모 다중 컴포넌트 프로젝트**
  — `/breakdown`이 더 적합합니다. `/ha`는 단일 작업 단위에 최적화되어 있습니다.

- **요구사항이 매우 모호한 경우**
  — 먼저 `/haqq`나 `/haqqq`로 질문을 통해 요구사항을 명확히 한 뒤 사용하세요.

---

**전체 참고 문헌**: Shinn (2023) Reflexion · Dhuliawala (2023) CoVe · Madaan (2023) Self-Refine · Press (2022) Self-Ask · Yao (2023) ToT · Zhou (2022) Least-to-Most · Wang (2023) Plan-and-Solve · Hu (2024) UoT · Dahl (2024) Legal Hallucinations
